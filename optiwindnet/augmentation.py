...
def iCDF_factory(
    ...
    def integral_inv(y):  # integral_inv(integral(x)) = x
        return (
            -4 * np.sqrt(4 * η**2 - np.pi * η * d_lb * y) + 8 * η - np.pi * d_lb * y
        ) / (np.pi * d_lb**2)
...
def normalize_site_single_oss(
    L: nx.Graph,
) -> tuple[
    np.ndarray[tuple[int, COLS], np.dtype[np.float64]],
    np.ndarray[tuple[COLS], np.dtype[np.float64]],
]:
...
def build_instance_graph(
    WTpos, boundary, name='', handle='unnamed', oss=None, landscape_angle=0
):
...
def _clears(
    RepellerC: nb.float64[:, :], repel_radius_sq: float, point: nb.float64[:]
) -> bool:
...
def _contains_np(polygon: nb.float64[:, :], pts: nb.float64[:, :]) -> nb.bool_[:]:
    ...
    m1 = (polygon[:, 1] > pts[:, None, 1]) != (polygon_rolled[:, 1] > pts[:, None, 1])
...
def poisson_disc_filler(
    ...
    i_len, j_len = np.ceil(width_height / cell_size).astype(np.int_)
    ...
    if len(points) < T:
        msg = (
            f'Only {len(points)} points generated (requested: {T}, itera'
...
def _poisson_disc_filler_core(
    ...
    j_len: int,
    cell_idc: nb.int64[:, :],
    BorderGrid: nb.float64[:, :],
    repel_radius_sq: float,
    repellers_scaled: nb.optional(nb.float64[:, :]),
    ...
    def no_conflict(p: int, q: int, point: nb.float64[:]) -> bool:
        ...
        return not (((point[np.newaxis, :] - points[ii]) ** 2).sum(axis=-1) < 2).any()
