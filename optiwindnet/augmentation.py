...
from .geometric import (
    IndexPairs, CoordPair, CoordPairs, area_from_polygon_vertices
)
...
def _get_border_scale_offset(BorderC: CoordPairs) -> tuple[CoordPair, float,
                                                          float, float]:
...
def _clears(RepellerC: CoordPairs, repel_radius_sq: float,
            point: CoordPair) -> bool:
...
def _contains_np(polyC: CoordPairs, pts: CoordPairs)\
    ...
    m1 = ((polyC[:, 1] > pts[:, None, 1])
          != (polyC_rolled[:, 1] > pts[:, None, 1]))
    slope = (vectors[:, 1]*(pts[:, None, 0] - polyC[:, 0])
             - vectors[:, 0]*(pts[:, None, 1] - polyC[:, 1]))
    m2 = slope == 0
    mask2 = (m1 & m2).any(-1)
...
def _poisson_disc_filler_core(
        T: int, max_iter: int, i_len: int, j_len: int,
        cell_idc: IndexPairs,
        BorderS: CoordPairs,
        repel_radius_sq: float,
        RepellerS: CoordPairs | None,
        rng: np.random.Generator) -> CoordPairs:
...
def poisson_disc_filler(T: int, min_dist: float, BorderC: CoordPairs,
                        RepellerC: CoordPairs | None = None,
                        repel_radius: float = 0.,
                        obstacles: list[CoordPairs] | None = None,
                        seed: int | None = None,
                        max_iter: int = 10000, plot: bool = False,
                        partial_fulfilment: bool = True,
                        rounds: int = 1) -> CoordPairs:
    ...
    if RepellerC is None or repel_radius == 0.:
        RepellerS = None
        repel_radius_sq = 0.
    else:
    ...
    if plot:
        ...
        ax.scatter(*np.nonzero(inside), marker='+', facecolor='tab:green',
                   s=7, lw=0.2)
        ax.scatter(*BorderS.T, marker='o', facecolor='none', s=10, lw=0.4,
                   edgecolor='tab:orange', alpha=0.6)
    ...
    for _ in range(rounds):
        ...
        points, iter_count = _poisson_disc_filler_core(
            T, max_iter, i_len, j_len, cell_idc, BorderS, repel_radius_sq,
            RepellerS, rng)
    ...
    if best_T < T:
        warn('Only %d points generated (requested: %d, efficiency requested: '
             '%.3f, max_iter: %d). Iterations per round: %s',
             best_T, T, efficiency, max_iter, iter_counts)

    return points*cell_size + offsetC


def turbinate(L: nx.Graph, T: int, d: float, *,
              root_clearance: float | None = None, plot: bool = False,
              max_iter: int = 100_000, rounds: int = 5) -> nx.Graph:
    ...
    TerminalS = poisson_disc_filler(
        T, d, BorderC=BorderS, RepellerC=RepellerS,
        repel_radius=(d if root_clearance is None else root_clearance),
        max_iter=max_iter, rounds=rounds, plot=plot,
    ) 
    T = TerminalS.shape[0]
    B = BorderS.shape[0]
    return L_from_site(
        ...
        **{k: v for k, v in L.graph.items() if k in
           ('R', 'handle', 'name', 'landscape_angle')},
    )
...
def iCDF_factory(T_min: int, T_max: int, η: float, d_lb: float)\
    ...
    def integral_inv(y: float) -> float:  # integral_inv(integral(x)) = x
        return ((-4*math.sqrt(4*η**2 - math.pi*η*d_lb*y) + 8*η - math.pi*d_lb*y)
                / (math.pi*d_lb**2))
