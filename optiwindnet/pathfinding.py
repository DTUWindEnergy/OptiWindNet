...
class PathFinder:
    ...
    def _advance_portal(
        ...
        side: int | None = None,
    ) -> Generator[tuple[float, tuple[int, int], bool], None, None]:
        P = self.P
        T = self.T
        ...
        next(traverser)
        if side is not None:
            d_ref, is_promising = traverser.send((portal, side, False))
        ...
        def walk_pinch(pinch_vert, border_vert):
            ...
            while still_pinched:
                ...
                constraint_edge = (
                    (pinch_vert, next_border)
                    if pinch_vert < next_border
                    else (next_border, pinch_vert)
                )
                assert constraint_edge in constraint_edges, (
                    'constraint_edge not in constraint_edges'
                )
        ...
        while True:
            ...
            for u, v, rot, irot in (
                ...
                if u in contour_primes:
                    ...
                    probe_edge = (u, probe_vert) if u < probe_vert else (probe_vert, u)
            ...
            if left_edge in constraint_edges and right_edge in edges_G_primed:
                # n is a pinch-node
                pinched, _left, _right = walk_pinch(n, left)
            elif right_edge in constraint_edges and left_edge in edges_G_primed:
                # n is a pinch-node
            ...
            if len(portals) == 2:
                ...
                if 0 <= n <= T:
                    ...
                    d_ref, is_promising = traverser.send(((left, n), 1, False))
    ...
    def _traverse_channel(
        ...
        wedge_end: list[int],
        bad_streak: int = 0,
    ) -> Generator[tuple, tuple[tuple[int, int], int, bool] | None, None]:
