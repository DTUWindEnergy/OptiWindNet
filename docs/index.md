...
{doc}`Set up <setup>` your Python environment and check the {doc}`Quickstart <notebooks/hi00_quickstart>` to begin using OptiWindNet.

For the concepts behind the tool, read {doc}`problem`, {doc}`data` and {doc}`methods` — these apply whichever API you use. Then pick an API: the {doc}`high_level_api` for most uses, or the {doc}`low_level_api` for fine-grained control. {doc}`overview` explains the difference, {doc}`howto` indexes both by task, and the {doc}`API Reference <autoapi/index>` documents every signature.
