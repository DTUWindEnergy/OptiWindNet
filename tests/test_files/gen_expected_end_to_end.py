...
# -*- coding: utf-8 -*-
"""
Generate expected graphs for DTU letters using repository-backed sites.

Simplifications:
- Sites are simple names (list/tuple).
- No validation around load_repository() or site existence.
- Routers are stored as specs (pickle-friendly).
"""
...
ROUTERS_1: Dict[str, Dict[str, Any]] = {
    ...
    "MILPRouter_cap10": r_spec("MILPRouter", {"solver_name": "ortools", "time_limit": 5, "mip_gap": 1e-3}, cables=10),
...
def generate_expected(output_path: Path) -> None:
    ...
    """
    print_header("Generating expected DTU letters (repo sites, no checks)")
    ...
    run_plan = [
        ("sites_1 × routers_1", SITES_1, ROUTERS_1),
        ("sites_2 × routers_2", SITES_2, ROUTERS_2),
        ("sites_3 × routers_3", SITES_3, ROUTERS_3),
    ]
